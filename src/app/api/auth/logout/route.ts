...
// Handle POST requests to clear the session cookie
export async function POST(request: Request) { // You can use Request or NextRequest
  try {
    console.log("API Logout: Attempting to clear session token cookie.");

    // Serialize using the imported function directly
    const sessionCookie = serialize( // <<< Use serialize directly
        'sessionToken',
        '',
        {
            httpOnly: true,
    ...
    // Return success response with the instruction to clear the cookie
