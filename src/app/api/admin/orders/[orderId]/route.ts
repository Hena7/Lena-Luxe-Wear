...
export async function GET(
  ...
  try {
    ...
    console.log(
      `API: Admin user ${payload.email} authenticated for order ${orderId}.`,
    );
  } catch (error) {
    return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
  }
  ...
  try {
    const order = await prisma.order.findUnique({
      ...
      include: {
        user: {
          // Include full user details needed for display
          select: { id: true, email: true, name: true, phoneNumber: true },
        },
        items: {
          // Include items and their product details
          include: {
            product: {
...
export async function PUT(
  ...
  console.log(
    `API: Received request to UPDATE status for order ${orderId} (Admin)`,
  );
  ...
  try {
    ...
    console.log(
      `API: Admin user ${payload.email} authenticated for order status update ${orderId}.`,
    );
  } catch (error) {
    return NextResponse.json({ message: "Unauthorized" }, { status: 401 });
  }
  ...
  try {
    ...
    if (!body.status || !Object.values(OrderStatus).includes(body.status)) {
      throw new Error(
        `Invalid or missing status value. Valid statuses are: ${Object.values(OrderStatus).join(", ")}`,
      );
    }
  } catch (error: any) {
    return NextResponse.json(
  ...
  try {
    const updatedOrder = await prisma.order.update({
      ...
      select: {
        // Return the updated status and timestamp
        id: true,
        status: true,
        updatedAt: true,
      },
    ...
    console.log(
      `API: Successfully updated order ${orderId} status to ${updatedOrder.status}`,
    );
