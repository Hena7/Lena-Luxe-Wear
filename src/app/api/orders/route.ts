...
export async function GET(request: NextRequest) {
  ...
  if (!token) {
    ...
    return NextResponse.json(
      { message: "Unauthorized: Login required to view orders." },
      { status: 401 },
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!token) {
    return NextResponse.json(
      { message: "Unauthorized: Must be logged in to place orders." },
      { status: 401 },
    );
  }
  ...
  for (const item of body.items) {
    if (
      ...
      return NextResponse.json(
        {
          message:
            "Invalid item data. Each item needs productId and a positive integer quantity.",
        },
        { status: 400 },
      );
    }
    ...
    const newOrder = await prisma.$transaction(
      async (tx) => {
        ...
        const createdOrder = await tx.order.create({
          ...
          include: {
            // Include created items in the response (optional)
            items: {
