...
export default function AdminOrderDetailPage({
  ...
  const fetchOrderDetails = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errData.message || `Error fetching order: ${response.status}`,
        );
      }
  ...
  const handleStatusChange = async (newStatus: OrderStatus) => {
    ...
    console.log(
      `Attempting to update order ${order.id} to status ${newStatus}`,
    );
  ...
  if (!order) {
    // Handles case after loading where order wasn't found or not authorized
    return (
  ...
  const formatCurrency = (amount: number) =>
    locale === "am" ? `${amount.toFixed(2)} ብር` : `$${amount.toFixed(2)}`;
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
              <div>
                ...
                <dd className="mt-1 text-gray-700 dark:text-gray-200 font-mono">
                  {order.id}
                </dd>
              </div>
              <div>
              ...
              <div>
                ...
                <dd className="mt-1 text-gray-700 dark:text-gray-200 font-medium">
            ...
            {order.user ? (
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-gray-700 dark:text-gray-200 font-mono text-xs">
                    {order.user.id}
                  </dd>
                </div>
              </dl>
                ...
                <li key={item.id} className="flex py-4">
                  <div className="flex-shrink-0">
                    <Image
                      ...
                      width={64}
                      height={64}
                      className="h-16 w-16 rounded-md object-contain border p-1 bg-white"
                    />
                  </div>
          ...
          <div className="sticky top-24 bg-gray-50 dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p
                  className={`mt-1 text-sm font-medium inline-flex items-center rounded-full px-3 py-1 ${getStatusColor(order.status)}`}
                >
                  {order.status}
                </p>{" "}
                {/* Use helper */}
              ...
              <div>
                ...
                >
                  {orderStatuses.map(
                    (
                      status, // Use defined statuses array
                    ) => (
