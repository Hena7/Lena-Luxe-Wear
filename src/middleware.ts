...
export async function middleware(request: NextRequest) {
  ...
  if (JWT_SECRET) {
    try {
      ...
      console.error("MIDDLEWARE: Failed to encode JWT_SECRET", e);
    }
  }
  ...
  if (!secretKey || secretKey.length === 0) {
    ...
    if (process.env.NODE_ENV === "production") {
      return new NextResponse("Internal Server Error: Configuration issue.", {
        status: 500,
      });
    }
  ...
  if (token) {
    try {
      ...
      if (
        ...
        userPayload = {
          // Assign only if valid structure
      ...
      console.warn(
        "MIDDLEWARE: Token verification failed (invalid/expired):",
  ...
  console.log(
    `MIDDLEWARE: User logged in: ${isUserLoggedIn}, Role: ${userRole}, Is Admin: ${isUserAdmin}`,
  );
  ...
  console.log(
    `MIDDLEWARE: Checking Admin Path (${pathname}): ${isAccessingAdminPath}`,
  );
  if (isAccessingAdminPath) {
    if (!isUserLoggedIn) {
      console.log(
        "MIDDLEWARE: Redirecting to login (Admin Path, Not Logged In).",
      );
    ...
    if (!isUserAdmin) {
      console.log(
        `MIDDLEWARE: Redirecting to home (Admin Path, Role is ${userRole}, Not Admin).`,
      );
  ...
  console.log(
    `MIDDLEWARE: Checking Protected Path (${pathname}): ${isAccessingProtectedPath}`,
  );
  if (isAccessingProtectedPath && !isUserLoggedIn) {
    console.log(
      "MIDDLEWARE: Redirecting to login (Protected Path, Not Logged In).",
    );
    return NextResponse.redirect(
      new URL(`/login?redirectedFrom=${pathname}`, request.url),
    );
  ...
  const isAccessingPublicOnlyPath = publicOnlyPaths.some((path) =>
    pathname.startsWith(path),
  );
  console.log(
    `MIDDLEWARE: Checking Public-Only Path (${pathname}): ${isAccessingPublicOnlyPath}`,
  );
  if (isAccessingPublicOnlyPath && isUserLoggedIn) {
    console.log(
      "MIDDLEWARE: Redirecting to home (Public-Only Path, Logged In).",
    );
